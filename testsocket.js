...
navigator.getUserMedia = navigator.getUserMedia ||  navigator.webkitGetUserMedia || navigator.mozGetUserMedia;
 ...
 var socket = io.connect('http://192.168.1.7:4000/');//hard coded ip
...
clientMsgButton.onclick=chat; // chat function
function chat(){
  //For demo testing validation
  if(clientMsgTxt.value!=''){
     socket.emit('chatmsg',clientMsgTxt.value); 
     msgDisPt.innerHTML = msgDisPt.innerHTML + '<span> Client:&nbsp;'+ clientMsgTxt.value +'</span><br/>';
  }
 
}
socket.on('chatmsg',function(msg){

msgDisPt.innerHTML = msgDisPt.innerHTML + '<span> Server:&nbsp;'+ msg +'</span><br/>';
});
//end
...
function iceCandidate(evt) {
  console.log(evt.candidate);
    if(evt.candidate){
      socket.emit('message', {type: "candidate", label: evt.candidate.sdpMLineIndex,id: evt.candidate.sdpMid,candidate: evt.candidate.candidate});
    }
